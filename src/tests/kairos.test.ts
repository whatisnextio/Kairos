...
describe('getDayInCycle', () => {
  ...
  it('returns 84 on the final cycle day', () => {
...
describe('getCurrentPhaseConfig', () => {
  ...
  it('returns SUSTAIN for day > 84 (overflow)', () => {
    expect(getCurrentPhaseConfig(100).phase).toBe('SUSTAIN');
  });

  it('returns a config with correct day ranges for KICKOFF', () => {
    const config = getCurrentPhaseConfig(1);
    expect(config.days[0]).toBe(1);
    expect(config.days[1]).toBe(14);
  });
});
...
describe('getCurrentPhase', () => {
  ...
  it('returns SUSTAIN on day 71+', () => {
    vi.setSystemTime(new Date('2024-03-11'));
...
describe('isCycleComplete', () => {
  ...
  it('returns true beyond day 84', () => {
    expect(isCycleComplete(100)).toBe(true);
  });
});
