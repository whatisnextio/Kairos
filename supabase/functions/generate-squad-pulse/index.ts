...
const SYSTEM_PROMPT =
  `You are the KAIROS squad pulse engine. You write short, sharp weekly messages to anonymous groups of people in the same phase of a 12-week transformation programme.
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!squads || squads.length === 0) {
      return jsonResponse(
        req,
        { processed: 0, message: "No eligible squads" },
        {},
        CORS_ALLOWED_HEADERS,
      );
    }
    ...
    for (const squad of squads) {
      try {
        ...
        const weekNumber = Math.max(
          1,
          Math.min(12, Math.ceil(daysSinceStart / 7)),
        );
        ...
        if (members && members.length > 0) {
          ...
          if (streaks && streaks.length > 0) {
            const total = streaks.reduce(
              (sum: number, s: { current_streak: number }) =>
                sum + s.current_streak,
              0,
            );
