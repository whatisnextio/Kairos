...
function buildLifechangerSignals(state: UserState): string[] {
  ...
  const grouped = state.recentCheckIns.reduce<
    Record<string, typeof state.recentCheckIns>
  >(
    (groups, checkIn) => {
  ...
  for (const [domain, items] of Object.entries(grouped)) {
    ...
    const score = items.reduce((sum, item) =>
      sum + completionWeight(item.status), 0) / items.length;
    ...
    if (score >= 0.75) {
      signals.push(
        `${label} is the strongest recent trend at ${
    ...
    if (missed >= 2) {
      signals.push(
        `${label} has ${missed} recent misses, so offer a recovery path.`,
      );
      break;
    }
  }

  const sortedVibes = [...state.vibeChecks].sort((a, b) =>
  ...
  if (sortedVibes.length >= 2) {
    ...
    if (Math.abs(last.rating - first.rating) >= 1) {
      signals.push(
        `Vibe moved from ${first.rating}/5 to ${last.rating}/5 across recent checks.`,
      );
    }
  }
  ...
  const streak = state.streaks
    .filter((item) =>
      item.current >= 3 || (item.longest >= 7 && item.current === 0)
    )
  ...
  if (noteSource?.notes) {
    const keyword = NOTE_KEYWORDS.find((item) =>
      noteSource.notes?.toLowerCase().includes(item)
    );
    signals.push(
  ...
  const selfConnectionOpen = ["METIME", "USTIME"].some((domain) =>
    grouped[domain]?.some((item) =>
      item.status === "Missed" || item.status === "Partial"
    )
  );
...
async function callClaude(userPrompt: string): Promise<{
  ...
  const costPence = Math.round(
    ((inputTokens * 0.8 + outputTokens * 4) / 1_000_000) * 100 * 100,
  );
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!PAID_TIERS.has(profile.tier) && !isSunday) {
      return jsonResponse(
        req,
        { error: "Free tier gets nudges on Sundays only", tier_gate: true },
        { status: 403 },
      );
    }
    ...
    const state: UserState = {
      tier: profile.tier,
      identityAnchorName:
        (profile.identity_anchors as { name: string } | null)?.name ??
    ...
    if (insertErr) {
      ...
      return jsonResponse(req, { error: "Failed to store nudge" }, {
        status: 500,
      });
    }
